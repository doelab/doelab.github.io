...
- Understanding the theory and mathematical concepts of the hardware folding technique for architecture optimization.
- Implementing the baseline (unfolded) FIR and IIR filter architectures using Verilog and SystemVerilog.
- Implementing the optimized folded filter architectures. 
- Running simulations to verify the functionality of both the baseline and folded filters.
...
In this lab assignment, you will design, optimize, and simulate FIR and IIR filters using Verilog or SystemVerilog.

Your first task is to implement the baseline (unfolded) FIR and IIR filters based on your initial specifications. Following the baseline design, you are required to apply the hardware folding technique to optimize your filter architectures.

To validate your work, you must build your own testbenches and use an audio sample to thoroughly verify the functionality of both the baseline and optimized designs through simulation.

Your primary goal is to complete the FIR and IIR filter designs and verify their operation through simulation using the testbench and input signals. You also need to synthesize the design to analyze and compare the design characteristics (such as resource utilization, maximum frequency, latency, and throughput) before and after implementing the folding technique and demo it on an FPGA for real-time testing.
...
|---|---|---|---|---|
| **Specification & Folding Factor** (1 pt) | No spec/folding calc; FIR fundamentals misunderstood | Spec partial; folding calc attempted but wrong/unjustified | Correct FIR understanding + derived spec + correct folding factor calc | Spec & folding derivation precise, justified, HW trade-offs discussed |
| **Baseline (Unfolded) RTL** (1 pt) | Incomplete, doesn't compile, or not simulated | Complete but sim/verification missing or fails | Baseline RTL aligned to spec, simulated & verified with at least 1 sample | Clean, robustly verified across multiple samples, well documented |
| **Folded Architecture** (2 pts) | Not implemented or non-functional | Partial; area/resource efficiency doesn't match folding factor | Folded architecture correct, area/resource-efficient per folding factor | Well-optimized, documented, efficient control logic beyond minimum |
| **Simulation Correctness** (1 pt) | Missing or incorrect results | Only one of baseline/folded verified | Both baseline & folded verified correct with samples/testbenches | Comprehensive, covers edge cases, clear baseline-vs-folded evidence |
| **Synthesis & Comparative Analysis** (2 pts) | Not synthesized, or no comparison | Only one design synthesized/analyzed; comparison shallow | Both synthesized; compares utilization, fmax, latency, throughput | Analysis thorough, explains resource/performance trade-offs of folding |
| **FPGA Live Demo** (3 pts) | No demo or folded filter fails on HW | Partial — real-time filtering unstable/incomplete | Live demo of folded filter, real-time performance & processing | Polished demo with periodic-wave + audio-wave, clear noise removal |
| **Bonus** (uncapped) | No bonus attempted | — | Parameterizable filter, audio demo, adaptive filter, own idea, suboptimal-result analysis, or social media content (e.g., short demo video/post) | Strong contribution (e.g., fully parameterizable/adaptive filter, compelling audio demo, polished social media content with engagement) or rigorous validated analysis |
